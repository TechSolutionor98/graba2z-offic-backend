...
router.get(
  ...
  asyncHandler(async (req, res) => {
    const pageSize = Number(req.query.limit) || 20; // Default limit to 20 products per page
    const page = Number(req.query.page) || 1; // Default to page 1

    const { category, subcategory, parentCategory, featured, search, brand, sortBy, minPrice, maxPrice } = req.query;
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      // Read Excel file
      const workbook = XLSX.read(req.file.buffer, { type: 'buffer' }); // Use req.file.buffer for memory storage
      const sheetName = workbook.SheetNames[0]
      const sheet = workbook.Sheets[sheetName]
      const rows = XLSX.utils.sheet_to_json(sheet)
      console.log("Excel rows loaded:", rows.length)

      // No need to unlink file as it's in memory storage
